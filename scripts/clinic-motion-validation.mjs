...
const out='docs/evidence/clinic-motion/validation';fs.mkdirSync(out,{recursive:true})
...
try{
 ...
 const result=await page.evaluate(async()=>{
  ...
  let staleBoundCulling=0;root.traverse(n=>{if(n.isSkinnedMesh&&n.frustumCulled)staleBoundCulling++})
  const snapshot=()=>{
   root.updateMatrixWorld(true);const bones=[],feet={}
   root.traverse(n=>{if(n.isBone)bones.push(...n.position.toArray(),...n.quaternion.toArray(),...n.scale.toArray());if(n.isSkinnedMesh){n.skeleton.update();if(n.name.includes('shoes01')){let min=Infinity;for(let i=0;i<n.geometry.attributes.position.count;i++){const v=n.position.clone();n.getVertexPosition(i,v);v.applyMatrix4(n.matrixWorld);min=Math.min(min,v.y)}feet[n.name]=min}}})
   return {bones,feet}
  }
  const samples=[]
  ...
  return {staleBoundCulling,reverseErrors:errors,holdError,freshError,feet:samples.map(({p,feet})=>({p,feet})),renderInfo:{calls:s.gl.info.render.calls,triangles:s.gl.info.render.triangles,geometries:s.gl.info.memory.geometries,textures:s.gl.info.memory.textures}}
 })
 ...
 assert.equal(result.staleBoundCulling,0,'travelling limbs must not disappear using cached bind-pose bounds')
