...
assert.equal(byId.object.eyebrow,'the clinical ai workspace')
assert.equal(byId.object.heading,'your clinic. wherever you practice.')
assert.match(byId.object.body,/designed to/)
assert.match(byId.object.body,/hematology/)
assert.match(byId.reveal.eyebrow,/clinical ai workspace/)
assert.doesNotMatch(source,/inside one box|clinic[- ]in[- ]a[- ]box|omnus appliance/i)
assert.doesNotMatch(fs.readFileSync('index.html','utf8'),/inside one box|clinic[- ]in[- ]a[- ]box/i)
